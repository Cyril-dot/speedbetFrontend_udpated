...
function LiveDot() {
  return (
    <span style={{ position: 'relative', display: 'inline-flex', width: 8, height: 8 }}>
      <span style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: '#ef4444', opacity: 0.6, animation: 'ping 1.4s cubic-bezier(0,0,0.2,1) infinite' }} />
...
function StatCard({ label, value }) {
  return (
    <div style={{ background: 'var(--surface-0)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
      <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-60)', marginBottom: 4 }}>{label}</div>
      <div style={{ fontFamily: 'monospace', fontSize: 28, fontWeight: 900, color: 'var(--text-100)', lineHeight: 1 }}>{value}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', marginBottom: 28 }}>
      ...
      <div style={{ overflowX: 'auto', marginLeft: -16, marginRight: -16, paddingBottom: 18 }} className="no-scrollbar">
        <div style={{ display: 'flex', gap: 10, paddingLeft: 16, paddingRight: 16, paddingTop: 4, minWidth: 'max-content' }}>
          {loading
...
export default function Live() {
  ...
  const avgMinute   = live.length ? Math.round(live.reduce((s, m) => s + (m.minute ?? 0), 0) / live.length) : null;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-10">
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <LiveDot />
            <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#ef4444' }}>In Play</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            ...
            {!loading && live.length > 0 && (
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', padding: '4px 10px', borderRadius: 999, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 24px', background: 'var(--surface-0)', border: '1px solid var(--border)', borderRadius: 16 }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>⚽</div>
            <div style={{ fontWeight: 700, fontSize: 16, color: 'var(--text-100)', marginBottom: 6 }}>No live matches right now</div>
            <p style={{ fontSize: 13, color: 'var(--text-60)', marginBottom: 20 }}>Check back soon or browse upcoming fixtures.</p>
            <Link to="/app/sports" style={{ display: 'inline-block', padding: '9px 20px', borderRadius: 8, background: 'var(--grad-primary)', color: '#fff', fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>
              See upcoming →
            </Link>
          </div>
        ) : (
